            ...
            if ($user) {
                ...
                if ($mfaGloballyEnabled
                    ...
                    if (data_get($user, 'mfa.registrationCompleted')) {
                        $mfaDeviceId = $request->cookie('mfa_remember_token');
                        $now = \Carbon\CarbonImmutable::now()->timestamp;
                        $hasValidSavedMfa = $mfaDeviceId && collect(data_get($user, 'mfa.savedDevices', []))->first(function($item) use($mfaDeviceId, $now) {
                            return data_get($item, 'id') === $mfaDeviceId
                                && data_get($item, 'expiration') >= $now;
                        });
                        if (!$hasValidSavedMfa) {
