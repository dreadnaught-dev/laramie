...
@if ($isVisibleOnEdit)
    <div class="field" data-field-key="{{ $fieldKey }}" data-field-type="{{ $field->type }}">
        ...
        <input type="hidden" id="{{ $field->id }}" name="{{ $field->id }}" value="{{ data_get($item, $field->id) }}" onchange="$(this).closest('.field').find('.hidden-value').text($(this).val())">
        @include('laramie::partials.fields.edit._help-text')
    </div>
@else
    <input type="hidden" id="{{ $field->id }}" name="{{ $field->id }}" value="{{ data_get($item, $field->id) }}">
@endif
