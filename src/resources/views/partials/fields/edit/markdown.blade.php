...
@section('input')
    <div class="columns">
        <div class="column is-12-desktop is-half-fullhd markdown-textarea">
            <textarea class="textarea markdown" rows="12" id="{{ $field->id }}" name="{{ $field->id }}" {!! $field->extra !!} {{ $field->required ? 'required' : '' }}>{{ data_get($item, $field->id . '.markdown') }}</textarea>
        ...
        <div class="column is-half is-hidden-touch is-hidden-desktop-only is-hidden-widescreen-only markdown-preview">
            <div class="content markdown-html">{!! data_get($item, $field->id . '.html') !!}</div>
        </div>
    </div>
@overwrite
