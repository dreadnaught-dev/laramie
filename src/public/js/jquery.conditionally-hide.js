(function($) {
    $.fn.conditionallyHide = function(options) {
        ...
        return this.each(function(i, item) {
            ...
            var elementToHideIdentifier =
                (elementToHide.attr("id") || elementToHide.attr("name") || elementToHide.attr("class")) +
                "  ::SHOW WHEN::  " +
                rule;

            if (settings.wrapperSelector != null) {
                var wrapperSelector = $e.is("[data-override-wrapper-selector]")
                    ? $e.data("overrideWrapperSelector")
            ...
            if (
                (target.length > 1 && comparisonOp(target.filter(":checked").val(), value)) || // radios
                (target.length == 1 &&
                    target.is('[type="checkbox"]') &&
                    comparisonOp(target.filter(":checked").val(), value)) || // checkboxes
            ...
            target.on("updated", function(e) {
                var target = $(e.target);
                if (
                    (target.is('[type="checkbox"]') && comparisonOp(target.filter(":checked").val(), value)) || // checkboxes
    ...
    $.fn.conditionallyHide.is_not_in = function(a, b) {
        var tmp = b.split(/\s*,\s*/);
        return tmp.indexOf(a) == -1;
    };
    $.fn.conditionallyHide.is_in = function(a, b) {
        var tmp = b.split(/\s*,\s*/);
        return tmp.indexOf(a) > -1;
    };
    $.fn.conditionallyHide.has_value = function(a, b) {
        return !/^(0)?$/.test(a);
    };
    $.fn.conditionallyHide.has_no_value = function(a, b) {
        return /^(0)?$/.test(a);
    };
    $.fn.conditionallyHide.less_than = function(a, b) {
        a = Number(a || 0);
        b = Number(b || 0);
        return a <= b;
    };
